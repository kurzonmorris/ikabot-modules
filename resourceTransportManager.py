...
def print_module_banner(mode_name=None, mode_description=None):
    """
    Print the Resource Transport Manager banner with optional mode name and description
    
    Parameters
    ----------
    mode_name : str, optional
        The current mode name (e.g., "Consolidate Resources")
    mode_description : str, optional
    ...
    if mode_name:
        print("║──────────────────────────────────────────────────────────║")
        # Center the mode name (60 chars wide)
        mode_line = f"║ {mode_name:^58} ║"
        print(mode_line)
        
        if mode_description:
            # Center the description (60 chars wide)
            desc_line = f"║ {mode_description:^58} ║"
            print(desc_line)
...
def evenDistributionMode(session, event, stdin_fd, predetermined_input, telegram_enabled):
    ...
    Uses ikabot's proven distribute_evenly algorithm
    """
    sys.stdin = os.fdopen(stdin_fd)
    config.predetermined_input = predetermined_input
    try:
        ...
        # Call ikabot's distribute_evenly function
...
def distribute_evenly(session, resource_type, cities_ids, cities):
    """
    COPIED VERBATIM FROM IKABOT - distributeResources.py lines 109-230
