...
def run_equilibrium_mode(session, event, stdin_fd, predetermined_input):
    ...
    # Check if Telegram is available (only if notifications requested)
    # Note: We skip the checkTelegramData call to avoid extra prompts
    # If Telegram isn't configured, sendToBot will simply fail silently
    if notification_mode < 3:
        # Just set a flag, don't actually check (avoids prompts)
        pass
    ...
    if run_hours == 0:
        ...
        if changes_made:
            ...
            for change in changes_made:
                # Extract just the city name and change from the notification string
        ...
        try:
            ...
            if notification_mode == 2:  # Notify on errors
...
def optimize_tavern_for_satisfaction(session, city, tavern, current_satisfaction):
    ...
    # Find optimal level: highest level where satisfaction stays above +5
    target_satisfaction = 5
    optimal_level = 0
    
    for level in range(len(sat_per_wine)):
        # Calculate what satisfaction would be at this level
        # Note: satPerWine gives the BOOST from wine, not the total
        wine_boost = sat_per_wine[level]
        # We need to figure out base satisfaction without wine
        base_satisfaction = current_satisfaction - sat_per_wine[current_level] if current_level < len(sat_per_wine) else current_satisfaction
        
        projected_satisfaction = base_satisfaction + wine_boost
