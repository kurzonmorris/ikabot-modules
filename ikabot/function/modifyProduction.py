...
def modifyTempleWorkers(session, event, stdin_fd, predetermined_input):
    """Assign priests in the temple of each chosen city.
    ...
    try:
        banner()
        city_ids, _ = ignoreCities(
            session, msg="In which cities do you want to set priests?"
        )
        ...
        for city_id in city_ids:
            ...
            temple_slot = next(
                (slot for slot in city["position"]
                 if slot.get("building") == "temple"),
                None,
            )
            ...
            url = (
                f"view=temple&cityId={city['id']}&position={position}"
                f"&backgroundView=city&currentCityId={city['id']}"
            ...
            session.post(params={
                ...
                "backgroundView": "city", "currentCityId": city["id"],
                "templateView": "temple", "actionRequest": actionRequest,
                "ajax": "1",
            })
