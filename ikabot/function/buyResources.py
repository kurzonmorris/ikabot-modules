...
def getOffers(session, city):
    ...
    for row in re.findall(r'<tr[^>]*>([\s\S]*?)</tr>', html):
        ...
        tipo_match = re.search(r'<img src="([^"]+\.png)"', row)
        speed_match = re.search(r'<td>(\d+)</td>', row)

        if not all([city_match, amount_match, price_match, href_match, tipo_match]):
            continue

        raw_amount = amount_match.group(1).replace('.', '').replace(',', '')
        tipo = tipo_match.group(1)

        if tipo == "//gf2.geo.gfsrv.net/cdn19/c3527b2f694fb882563c04df6d8972.png":
            tipo = "wood"
        elif tipo == "//gf1.geo.gfsrv.net/cdnc6/94ddfda045a8f5ced3397d791fd064.png":
            tipo = "wine"
        elif tipo == "//gf3.geo.gfsrv.net/cdnbf/fc258b990c1a2a36c5aeb9872fc08a.png":
            tipo = "marble"
        elif tipo == "//gf2.geo.gfsrv.net/cdn1e/417b4059940b2ae2680c070a197d8c.png":
            tipo = "glass"
        elif tipo == "//gf1.geo.gfsrv.net/cdn9b/5578a7dfa3e98124439cca4a387a61.png":
            tipo = "sulfur"
        else:
            continue
