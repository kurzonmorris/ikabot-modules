...
def _seat_dir():
    """Directory holding the machine-wide worker "seat" locks.

    Upstream puts these in /tmp, which is per-container: with one container
    per account each instance gets its own seat directory, every instance
    then sizes itself to the full core count, and the coordination silently
    does nothing — exactly the contention it exists to prevent.

    We default to IKABOT_DATA_DIR instead, which is already the shared volume
    in a multi-account Docker setup (the same one the status files use), so
    coordination works across containers with no extra mount. Override with
    IKABOT_DECAPTCHA_SEAT_DIR if the data dir is not shared, or point it at
    /tmp to get upstream's per-machine behaviour.

    Note the locks are flock-based, so the directory must be on a filesystem
    that implements flock properly: a normal Docker volume or bind mount is
    fine, NFS generally is not.
    """
    override = os.getenv("IKABOT_DECAPTCHA_SEAT_DIR")
    if override:
        return override
    try:
        from ikabot.config import IKABOT_DATA_DIR
        return os.path.join(IKABOT_DATA_DIR, "decaptcha_seats")
    except Exception:
        import tempfile
...
def _cgroup_available_mb():
    """Free RAM inside this container's cgroup, or None if not constrained.

    /proc/meminfo reports the HOST's memory even inside a container, so a
    container capped at 1 GB on a 32 GB host would otherwise size its worker
    pool from 32 GB and get OOM-killed. Read the cgroup's own limit instead
    and return whatever is still unused inside it.
    """
    def _read(path):
        try:
            with open(path) as f:
    ...
    if limit is not None and usage is not None:
        if limit == "max":
            return None                      # not memory-constrained
        try:
            return max(0.0, (int(limit) - int(usage)) / 1048576.0)
        except ValueError:
            pass
    ...
    if limit is not None and usage is not None:
        try:
            limit_i, usage_i = int(limit), int(usage)
            # v1 signals "unlimited" with a huge sentinel rather than a word.
            if limit_i >= (1 << 62):
                return None
            return max(0.0, (limit_i - usage_i) / 1048576.0)
        except ValueError:
            pass
...
def _available_mb():
    ...
    Takes the smaller of the host figure and this cgroup's, so the pool is
    sized to whichever actually constrains us.
    """
    host = None
    try:
    ...
    if host is None:
        try:
            ...
            host = status.ullAvailPhys / 1048576.0
        except Exception:
            pass

    cg = _cgroup_available_mb()
    candidates = [v for v in (host, cg) if v is not None]
...
def _cgroup_cpu_limit():
    """CPU cores this cgroup may use, or None when unconstrained.

    os.cpu_count() reports the HOST's cores inside a container, so `--cpus=2`
    on a 16-core host would otherwise plan for 16 workers that then fight over
    two cores' worth of quota.
    """
    def _read(path):
        try:
            with open(path) as f:
    ...
    if quota is not None and period is not None:
        try:
            ...
            if quota_i > 0 and period_i > 0:
                return max(1, int(quota_i // period_i))
        except ValueError:
            pass
...
def _usable_cores():
    ...
    Combines the CPU affinity mask (which honours --cpuset-cpus) with the
    cgroup CFS quota (which honours --cpus). Falls back to os.cpu_count().
    """
    try:
...
def _init_mp():
    ...
    try:
        ...
        _MP_FORK = False
    _MP_CORES = max(2, min(cores, _physical_cores(cores)))
