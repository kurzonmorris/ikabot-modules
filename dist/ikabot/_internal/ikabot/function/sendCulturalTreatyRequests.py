...
def _send_treaty_request(session, current_city_id, target_player_id):
    """
    Send a cultural treaty request to the target player (msgType=77).
...
def sendCulturalTreatyRequests(session, event, stdin_fd, predetermined_input):
    ...
    try:
        ...
        if last_outgoing is not None:
            ...
            print(
                "Could not determine the last outgoing message time. Outbox shows {} outgoing message(s) in the last 5 minutes.".format(
                    len(initial_recent_outgoing),
                )
            )
        ...
        for i, (player_id, info) in enumerate(to_send, start=1):
            ...
            while True:
                ...
                merged_outgoing, _ = _merge_outgoing_timestamps(
                    recent_outbox,
                    bot_send_timestamps,
                )
                ...
                print(
                    "  [Outbox pacing] Waiting {}s because {} outgoing message(s) were detected in the last 5 minutes.".format(
                        wait_seconds,
